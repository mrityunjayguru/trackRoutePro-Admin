    ...
    <div>
      ...
      <div className="flex justify-between px-1">
        <h1 className="text-[#000000] text-xl">
          {SingleSubscriber?.subscribeType || 'No Subscription Type Available'}
        </h1>
        <div>
          {createdByUser ? (
            createdByUser.role === 'Dealer' ? (
              <>
        ...
        <div className="flex gap-1">
          {userDetails && loginUser.role!="Dealer" ? (
            <p className="text-[#9F9EA2] text-sm font-normal">
              Updated By: {userDetails?.Name || 'N/A'} (
              {userDetails?.uniqueCode || 'N/A'}) on{' '}
              {formatDateToDDMMMYYYYwithTime(userDetails?.time) || 'N/A'}
            </p>
          ) : (
            null
          )}
          {/* SVG Icon for opening the popup */}
          {loginUser.role!="Dealer"?(<svg
            ...
            onClick={togglePopup} // Add the click handler here
          >
