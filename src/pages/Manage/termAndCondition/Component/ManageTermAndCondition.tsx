...
import { createTermAndConditions, getTermAndCondition, updateTermAndConditions } from '../../../../api/termAndCondition';

function ManageTermAndCondition() {
  ...
  const getRecords = () => {
    const payload: any = {};
    dispatch(getTermAndCondition(payload));
  };
  ...
  const submitRecord = () => {
    if (validateDescription(description)) {
      ...
      if (records.length > 0) {
        ...
        dispatch(updateTermAndConditions({ _id: recordId, ...payload }))
          .then((result) => {
        ...
        dispatch(createTermAndConditions(payload))
          .then((result) => {
            if (result.meta.requestStatus === 'fulfilled') {
              getRecords()
      ...
      <div className="px-2 py-5">
        <div className="mb-4 w-1/2">
          <textarea
            ...
            required
            className="w-full pl-10 pr-10 py-2 border rounded-lg focus:ring-2 focus:ring-[#D9E821] focus:outline-none min-h-[250px]"
            rows={4}
          />
...
export default ManageTermAndCondition;
