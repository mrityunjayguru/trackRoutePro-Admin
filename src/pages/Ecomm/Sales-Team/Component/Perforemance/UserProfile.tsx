...
const UserProfile: React.FC<UserProfileProps> = ({ record }) => {
  return (
    <div className=" space-x-4 p-4 rounded-xl bg-[#F0F4FA] border border-gray-200">
      <div className="flex-shrink-0 flex gap-5 bg-[#ffffff] items-center">
        <img
      ...
      <div className="flex-grow">
        ...
        <div className="mt-3 text-gray-700 text-sm sm:text-base">
          ...
          <p className='text-[#1A1D1F] font-[500]'><span className="font-medium text-[#585859]">Email:</span> {record?.email || "N/A"}</p>
          <p className='text-[#1A1D1F] font-[500]'><span className="font-medium text-[#585859]">Mobile:</span> {record?.phone || "N/A"}</p>
        </div>
      </div>
    </div>
  );
