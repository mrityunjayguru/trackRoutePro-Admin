...
import { setAllSubscriber,singleSubscriber,groupSubscriber,singleDelearSubscriber } from "../../store/subscriber";
...
export const singleDelearSubscribers = createAsyncThunk<boolean, Payload>(
  APIName.subscribers,
  async (payload, thunkAPI) => {
    try {
      thunkAPI.dispatch(singleDelearSubscriber(payload));
    } catch (err:any) {
      if(err.status==401){
        localStorage.removeItem("token")
        GetMessage("warning", "Unauthorized");
...
export const delearSubscribers = createAsyncThunk<boolean, Payload>(
  APIName.subscribers,
  async (payload, thunkAPI) => {
    try {
      const data:any = await userRepo.delearSubscriber(payload);
...
export const getDelearRecord = createAsyncThunk<boolean, Payload>(
  APIName.subscribers,
  async (payload, thunkAPI) => {
    try {
      const data:any = await userRepo.getDelearRecord(payload);
...
export const getSubscriberRecordAddEdByDelear = createAsyncThunk<boolean, Payload>(
  APIName.subscribers,
  async (payload, thunkAPI) => {
    try {
      const data:any = await userRepo.subscriberByDelear(payload);
...
export const getDelearSuports = createAsyncThunk<boolean, Payload>(
  APIName.subscribers,
  async (payload, thunkAPI) => {
    try {
      const data:any = await userRepo.getDelearSuport(payload);
